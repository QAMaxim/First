...
#удобно использовать, например, когда необходимо чтобы кажое слово начиналось с заглавной буквы
...
print (mystr.center(40, '*'), ('- метод .center (помешают нашу строку в центр и заполняет указанную длину указанным символом)'))
...
параметр tabsize.'''
mystr2 = str ('Я ДаВнО\t БыЛ нА МоРе.') #удобно использовать для выравнивания текста и улучшения читаемости
print (mystr2.expandtabs(tabsize=5), ('- метод .expandtabs (заменяет символы табуляции указанным количеством пробелов, что может улучшbть читаемость)'))
